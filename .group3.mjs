...
const open = async (token, viewport) => {
  ...
  p.on("console", (m) => {
    const t = m.text();
    if (/VideoCall|realtime|Realtime|error|failed|denied/i.test(t))
      console.log("    [pg]", t.slice(0, 150));
  });
  p.on("pageerror", (e) =>
    console.log("    [pageerror]", String(e).slice(0, 150)),
  );
...
// Host starts
await a.p
  .locator('button[title*="ideo call"], button[title*="Voice call"]')
  .first()
  .click();
await a.p.waitForTimeout(1500);
await a.p
  .locator('[data-id="call-lobby-join"]')
  .click()
  .catch(() => {});
...
const state = await api("conversationVideoState", P[0][1], {
  conversationId: CONV,
});
console.log("— server roster —");
for (const [id, p] of Object.entries(state.participants ?? {})) {
  console.log(
    `  ${id.slice(0, 10)} isBot=${p.isBot} session=${p.sessionId ? "yes" : "NO"} tracks=${(p.tracks ?? []).length}`,
  );
}
console.log(
  "  active:",
...
await a.p.screenshot({
  path: `.shots/group3-${process.argv[3] === "mobile" ? "mobile" : "desktop"}.png`,
});
