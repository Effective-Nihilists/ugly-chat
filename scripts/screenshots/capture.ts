...
const HUMAN_DM_ID = directConversationId(
  SCREENSHOT_USER_ID,
...
async function injectProdAuthCookie(
...
async function shoot(
  ...
  try {
    ...
    await page.goto(`${BASE}${t.route}?screenshot=1${deviceParam}`, {
    ...
    await page.waitForFunction(
      () =>
        (document.querySelector("#root, #app, body > div")?.textContent?.trim()
          .length ?? 0) > 20,
      undefined,
      { timeout: 25000 },
    );
    ...
    if (t.call) {
      ...
      await page
        .locator('[data-id="video-call"]')
        .first()
        .waitFor({ state: "visible", timeout: 12000 });
      await page.waitForTimeout(3000);
      await page.screenshot({
        path: path.join(OUT_DIR, `${t.key}-${device}.png`),
      });
      res.ok = true;
...
async function main(): Promise<void> {
  ...
  for (const t of TARGETS) {
    for (const device of DEVICES) {
      ...
      results.push(r);
      console.log(
        `[shot] ${r.key}-${r.device}: ${r.ok ? "PASS" : "FAIL " + (r.error ?? "")}`,
      );
    }
  }
  ...
  for (const r of results) {
    console.log(
      `${r.ok ? "PASS" : "FAIL"}  ${r.key}-${r.device}${r.error ? "  " + r.error : ""}`,
    );
  }
