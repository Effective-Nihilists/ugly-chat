...
const UB_SECRET = JSON.parse(
  fs.readFileSync(
    os.homedir() + "/.ugly-studio/projects/RpK_ri0bJB/publish-state.json",
  ),
).neon.anonKey;
...
async function remoteOk(page) {
  return page.evaluate(async () => {
    ...
    for (let i = 0; i < 80; i++) {
      ...
      if (v && v.videoWidth > 0 && v.readyState >= 2 && flow.videoFrames > 0) {
        ...
        return {
          ...
          w: v.videoWidth,
          h: v.videoHeight,
          framesDecoded: flow.videoFrames,
          bytes: flow.bytes,
    ...
    return {
      ...
      w: v?.videoWidth ?? 0,
      flow,
      ms: v?.srcObject ? v.srcObject.getTracks().map((t) => t.kind) : null,
    };
  });
}
...
async function lobbyJoin(page, tag) {
  ...
  await page
    ...
    .catch(() => console.log(`${tag}> lobby Join stayed disabled`));
...
(async () => {
  ...
  console.log(
    "setup conversationCreate:",
    await api(tokenA, "conversationCreate", {
      id: ROOM,
      type: "group",
      title: "E2E Video",
      mode: "public",
  ...
  console.log(
    "setup B join:",
    await api(tokenB, "conversationJoin", { conversationId: ROOM }),
  );
  ...
  const mk = async (token, tag) => {
    ...
    page.on("console", (m) => {
      const t = m.text();
      if (/\[VC\]|VideoCall|negotiate/i.test(t))
        console.log(`${tag}>`, t.slice(0, 220));
    });
    page.on("pageerror", (e) =>
      console.log(`${tag}!pageerror`, String(e).slice(0, 200)),
    );
    return page;
  };
  ...
  const authA = await pa.evaluate(() => ({
    tok: !!window.__AUTH_TOKEN__,
    sub: window.__AUTH_TOKEN__
      ? JSON.parse(atob(window.__AUTH_TOKEN__.split(".")[1])).sub
      : null,
  }));
  ...
  await pb.waitForSelector('[data-id="incoming-call-accept"]', {
    timeout: 20000,
  });
  ...
  console.log(
    "roster:",
    (await api(tokenA, "conversationVideoState", { conversationId: ROOM }))
      .body,
  );
