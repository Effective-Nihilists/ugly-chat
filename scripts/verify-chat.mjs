...
try {
  ...
  const mA = await rpc(tokenA, "conversationMessageCreate", {
    ...
    message: { text: "hello from A", markdown: "hello from A" },
  });
  ...
  const mB = await rpc(tokenB, "conversationMessageCreate", {
    conversationId: roomId,
    message: { text: "hi from B" },
  });
  assert(
    mB && (mB._id || mB.id),
    "B message created (auto-joined public room)",
  );

  // 4. Both messages persisted (ground truth — same rows trackDocs serves)
  assert(
    dbMessageCount(roomId) === 2,
    `2 messages persisted (got ${dbMessageCount(roomId)})`,
  );
  ...
  await rpc(tokenA, "conversationMessageReact", {
    conversationId: roomId,
    messageId: shortId(mB._id || mB.id),
    reaction: "heart",
  });
  ...
  assert(
    parseInt(reactCount, 10) >= 1,
    `reaction persisted (rows=${reactCount})`,
  );

  // 6. A deletes its own message → count drops to 1
  await rpc(tokenA, "conversationMessageDelete", {
    conversationId: roomId,
    messageId: shortId(mA._id || mA.id),
  });
  assert(
    dbMessageCount(roomId) === 1,
    `after delete, 1 visible message (got ${dbMessageCount(roomId)})`,
  );
  ...
  await rpc(tokenA, "conversationMessageCreate", {
    conversationId: botRoom,
    message: { text: "hello bot", markdown: "hello bot" },
  });
  const botMsgCount = () =>
    parseInt(
      execSync(
  ...
  let call = await rpc(tokenA, "conversationVideoBotJoin", {
    conversationId: vroom,
    botId: "bot-ugly",
  });
  const n = (c) => Object.keys(c?.participants ?? {}).length;
  assert(
    call.active && n(call) === 3,
    `call roster has A + B + bot (got ${n(call)})`,
  );
  assert(
    call.participants["bot-ugly"]?.isBot === true,
    "bot is a fake-call participant",
  );
