...
const SCREENSHOT_PARTNER_USER_ID =
  process.env["SCREENSHOT_PARTNER_USER_ID"] ??
  "5c0e5c0e-0000-4000-8000-5c0e5c0e5c0f";
const SCREENSHOT_NAME = "Alex Rivera";
const SCREENSHOT_AVATAR =
  "https://api.dicebear.com/9.x/thumbs/png?seed=alex-rivera&size=256";
const PARTNER_NAME = "Sam Rivera";
const PARTNER_AVATAR =
  "https://api.dicebear.com/9.x/thumbs/png?seed=sam-rivera&size=256";
...
function publishState(repoDir: string): {
  ...
  if (!s.neon?.connectionString || !s.neon.anonKey)
    throw new Error(`Incomplete neon state in ${f}`);
...
async function main(): Promise<void> {
  ...
  const base =
    readUglyApp(repo).deployTarget?.customDomainUrl ?? "https://ugly.chat";
  ...
  const api = async (
    ...
    if (!res.ok || json.error)
      throw new Error(`${name} failed: ${res.status} ${json.error ?? ""}`);
    return json.result;
  };

  // ── 0. Local userPublic rows so chat resolves display names + avatars ───────
  const upsertPublic = async (
    id: string,
  ...
  await upsertPublic(UGLY_BOT_ID, {
    name: UGLY_BOT.name,
    isBot: true,
    avatar: UGLY_BOT_AVATAR_URL,
  });
  await upsertPublic(SCREENSHOT_USER_ID, {
    name: SCREENSHOT_NAME,
    avatar: SCREENSHOT_AVATAR,
  });
  await upsertPublic(SCREENSHOT_PARTNER_USER_ID, {
    name: PARTNER_NAME,
  ...
  await api("conversationMessageCreate", {
    ...
    message: {
      text: "Roast my New Year resolutions: gym 5x a week and read 50 books.",
    },
  });
  await api(
  ...
  // ── 2. Human DM (no bots) ─────────────────────────────────────────────────────
  const humanDmId = directConversationId(
    SCREENSHOT_USER_ID,
    SCREENSHOT_PARTNER_USER_ID,
  );
  ...
  const humanThread: Array<{ text: string; tok: string }> = [
    {
      text: "hey, you still want to grab dinner before the launch tonight?",
      tok: userToken,
    },
    {
      text: "yes please, I am starving. 7pm at the ramen place?",
      tok: partnerToken,
    },
  ...
  for (const m of humanThread) {
    await api(
      "conversationMessageCreate",
      { conversationId: humanDmId, message: { text: m.text } },
      m.tok,
    );
    await sleep(1500);
  }
  ...
  await api(
    ...
    {
      conversationId: GROUP_ID,
      message: { text: "I got it. logs dashboard is up on my second monitor." },
    },
    partnerToken,
  );
  await api(
    "conversationMessageCreate",
    {
      conversationId: GROUP_ID,
      message: {
        text: 'Cute. A deploy with a plan. I give it eleven minutes before someone says "works on my machine."',
      },
    },
    botToken,
  );
