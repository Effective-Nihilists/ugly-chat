...
const creds = JSON.parse(
  fs.readFileSync(os.homedir() + "/.config/ugly-app/infra/uglybot.json"),
);
const PROD = `postgresql://${creds.dbUser}:${creds.dbPwd}@localhost:15432/uglybot`;
const NEON = JSON.parse(
  fs.readFileSync(
    os.homedir() + "/.ugly-studio/projects/11tm1kplpe/publish-state.json",
  ),
...
(async () => {
  ...
  for (;;) {
    const { rows } = await prod.query(
      `SELECT _id,data,created,updated,version FROM "userPublic" WHERE _id>$1 ORDER BY _id LIMIT $2`,
      [after, BATCH],
    );
    ...
    rows.forEach((r, i) => {
      const b = i * 5;
      v.push(`($${b + 1},$${b + 2}::jsonb,$${b + 3},$${b + 4},$${b + 5})`);
      pr.push(
        r._id,
    ...
    await neon.query(
      `INSERT INTO "userPublic" (_id,data,created,updated,version) VALUES ${v.join(",")} ON CONFLICT (_id) DO UPDATE SET data=EXCLUDED.data,updated=EXCLUDED.updated`,
      pr,
    );
    done += rows.length;
  ...
  for (const r of bots) {
    ...
    const pub = {
      ...
      ...(d.imageUri ? { imageUri: d.imageUri } : {}),
      ...(d.avatarId ? { avatarId: d.avatarId } : {}),
      ...(d.voiceId ? { voiceId: d.voiceId } : {}),
    };
    await neon.query(
      `INSERT INTO "userPublic" (_id,data,created,updated,version) VALUES ($1,$2::jsonb,now(),now(),1) ON CONFLICT (_id) DO NOTHING`,
      [r._id, JSON.stringify(pub)],
    );
    bdone++;
  }
  console.log("bot names added:", bdone);
  const tot = (await neon.query(`SELECT count(*)::int n FROM "userPublic"`))
    .rows[0].n;
