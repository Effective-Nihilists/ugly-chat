...
**This is not `bot-swarm`.** `bot-swarm` evolves a _blank canvas_ — it invents what to
build and demands ambitious redesigns. Eval-swarm evaluates a _finished product_: does
...
the real feature, on real data, and _see_ the result with its own eyes produces
...
is charged to _your_ (the caller's) ugly.bot account via `billToUserId`, not to a fake
...
6. **Does the app's machinery actually WORK, or just not crash?** A broken environment
   ...
   Before dispatching, prove each capability the personas will exercise is _live_, not
...
feedback` / `/fix-feedback` read. Have every persona prefix its message with `[eval-swarm]`
  ...
  fixer _can_ read these — no in-context handoff needed.)
...
2. Invent **3–5 personas**, each with a distinct _goal_ and _tolerance_, that between them
...
3. For each persona classify its data need:
   - **create-persona** — its usage _generates_ the data (a writer starting a story). No
     seed required; the act of creating is itself the test.
   - **consume-persona** — it needs _existing_ data to work on (a reader needs a
...
4. **Always include a first-run newcomer.** One persona must arrive COLD — has never used
   ...
   make-or-break moment for adoption (a powerful tool that's confusing to _start_ loses users
...
personas whose _patience_ differs, not just their task.
...
_(Optional repeatability: an app may ship `bots/eval-swarm.json` — `{ personas, seed,
model }`. If present, use it instead of inventing. See Config.)_
...
>    delighted you? Ease-of-use is a first-class finding — a feature that _works_ but is
...
`/fix-feedback` and `ugly-app feedback` read **prod**, so they will _not_ see these local
...
Eval-swarm is a _loop_, not a one-shot: run → rate → fix → re-run → confirm the rating
...
- **Deploy before re-running a prod-target swarm** — personas test the _deployed_ build,
...
- **A billed re-run is the user's call.** After fixing, you may have _directly verified_
...
figure out how to _reach_ is a real defect, not a nit.

| Stars                   | Meaning                                                                                            | Gate                                                                                    |
| ----------------------- | -------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| ★☆☆☆☆ **1 — Broken**    | A core feature fails; a real user cannot accomplish the primary job.                               | Any blocker on a _primary_ feature, or most personas' verdict = "broken".               |
...
| ★★★☆☆ **3 — Usable**    | Core jobs work but with real friction, wrong/empty AI output, or a broken _secondary_ feature.     | Any blocker/high on a secondary feature, or mixed persona verdicts.                     |
...
matches and _every_ capture looks blank, so the personas file fabricated render bugs. In
