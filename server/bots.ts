 ...
 */
export function parseTextGenResponse(data: unknown): {
  text: string;
  usage: MsgTelemetry;
} {
  const d = data as Record<string, unknown> | null | undefined;
  const msg = (d?.message ??
    (d?.result as Record<string, unknown> | undefined)?.message) as
  ...
  const u = (d?.usage ??
    (d?.result as Record<string, unknown> | undefined)?.usage ??
...
async function uglyBotTextGen(
  ...
  const env =
    (globalThis as { process?: { env?: Record<string, string | undefined> } })
      .process?.env ?? {};
  const base =
    env.UGLY_BOT_LOCAL === "1"
      ? "http://localhost:3000"
      : (env.UGLY_BOT_URL ?? "https://ugly.bot");
  const userToken = getUserToken();
  const endpoint = userToken
    ? `${base}/v1/ai/user-billed/text`
    : `${base}/v1/ai/text`;
...
export function parseImageGenResponse(data: unknown): string {
  ...
  const b64 =
    (typeof d.base64 === "string" ? d.base64 : undefined) ??
    (typeof result?.base64 === "string" ? result.base64 : undefined);
  if (b64) {
    const mime =
      (typeof d.mime === "string" && d.mime) ||
      (typeof result?.mime === "string" && result.mime) ||
      "image/png";
...
async function persistGeneratedImage(
  ...
  try {
    ...
    return await storage.put(
      "public",
      imageKey(userId, parsed.mime),
      parsed.bytes,
      parsed.mime,
    );
  } catch (err) {
    console.error(
      "[bots] image upload to R2 failed; keeping inline data url",
      err,
    );
    return src;
  }
 ...
 */
export function imageFailureReply(err: unknown): {
  reply: string;
  color?: string;
} {
  ...
  if (/\b(400|safety|policy|nsfw|blocked|rejected|content)\b/i.test(msg)) {
    return {
      reply:
        "The image model rejected that prompt. Try rephrasing it and send again.",
    };
  }
  if (/\b(429|rate|busy|overloaded)\b/i.test(msg)) {
    return {
      reply:
        "The image service is busy right now — wait a moment, then send your prompt again.",
    };
  }
  return {
    reply:
      "The image service hiccuped and couldn't finish that one — send your prompt again to retry.",
  };
}
...
export const BOTS: Record<string, BotDef> = {
  ...
  "bot-search": {
    id: "bot-search",
    name: "Search",
    systemPrompt: "You search the web and answer with cited sources.",
    model: "deepseek_v4_flash",
  },
};
...
export const botUser = (
  id: string,
): { _id: string; name: string; isBot: true } | null =>
...
export async function getBotConfig(
  ...
  if (doc) {
    return {
      ...
      buttons:
        (doc.buttons as { label: string; prompt: string }[] | undefined) ?? [],
...
async function botParticipants(
  ...
  const convUsers =
    (conv?.users as Record<string, { isBot?: boolean }> | undefined) ?? {};
...
export async function triggerBotReplies(
  ...
  for (const botId of botIds) {
      ...
      await conversationMessageCreate(
        {
          ...
          message: {
            text: ORPHAN_REPLY,
            markdown: ORPHAN_REPLY,
            color: "error",
          },
        },
        botId,
      );
      await bumpListForMessage(db, conversationId, ORPHAN_REPLY, botId).catch(
        (err: unknown) => {
    ...
    if (botId === SEARCH_BOT_ID || mode === "search") {
      ...
      try {
        ...
        await conversationMessageCreate(
          {
            conversationId,
            message: {
              text: SEARCH_UNAVAILABLE_REPLY,
              markdown: SEARCH_UNAVAILABLE_REPLY,
              color: "error",
            },
          },
          botId,
        ).catch((e: unknown) => {
          console.error("[bots] search fallback post failed", e);
        });
        await bumpListForMessage(
          ...
          console.error("[bots] list denorm failed", e);
        });
      }
      continue;
    }
    ...
    if (mode === "image") {
      ...
      if (!prompt) {
        ...
        try {
          ...
          console.warn(
            `[bots] imageGen failed for ${botId}:`,
            (err as Error).message,
          );
      ...
      try {
        ...
        console.warn(
          `[bots] textGen failed for ${botId} (status ${status ?? "?"}):`,
          (err as Error).message,
        );
    ...
    await bumpListForMessage(db, conversationId, reply, botId).catch(
      (err: unknown) => {
        console.error("[bots] list denorm failed", err);
      },
    );
