...
describe("sendInviteEmail", () => {
  it("mints a short share link and POSTs the built message", async () => {
    ...
    const shareLinkFn = vi.fn(
      async () => "https://ugly.bot/l/abc123",
    ) as unknown as typeof import("ugly-app/server/adapter/workers").shareLink;
    const env = {
      EMAIL_SEND_URL: "https://send.ugly.bot",
      EMAIL_SEND_TOKEN: "tok",
      APP_URL: "https://ugly.chat",
    };
    ...
    expect(shareLinkFn).toHaveBeenCalledWith(
      expect.objectContaining({
        requireAuth: true,
        target: expect.stringContaining("grp-1"),
      }),
    );
