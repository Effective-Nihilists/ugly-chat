...
describe("sanitizeHistoryContent", () => {
  ...
  it("collapses a hosted-URL image too (a text model cannot use the URL)", () => {
    expect(sanitizeHistoryContent("![](https://blob.ugly.bot/x/y.webp)")).toBe(
      "[image]",
    );
  });

  it("nukes a bare base64 data-URI not wrapped in markdown", () => {
    const out = sanitizeHistoryContent(
      "here: data:image/png;base64," + "Q".repeat(1000),
    );
  ...
  it("handles mixed text + image in one message", () => {
    const out = sanitizeHistoryContent(
      "here you go ![sunset](data:image/png;base64,ZZZZ) enjoy",
    );
...
describe("imageFailureReply", () => {
  ...
  it("names a busy service", () => {
    expect(imageFailureReply(new Error("HTTP 429 rate limit")).reply).toMatch(
      /busy/i,
    );
  });
  ...
  it("always gives a retry path, even for an unknown error", () => {
    expect(imageFailureReply(new Error("weird")).reply).toMatch(
      /send your prompt again/i,
    );
