...
describe("parseImageGenResponse", () => {
  it("returns a hosted url when present", () => {
    expect(parseImageGenResponse({ url: "https://blob/x.png" })).toBe(
      "https://blob/x.png",
    );
    expect(
      parseImageGenResponse({ result: { imageUrl: "https://blob/y.png" } }),
    ).toBe("https://blob/y.png");
  });
  it("builds a data: URL from the user-billed base64 response (the real prod shape)", () => {
    ...
    const out = parseImageGenResponse({
      type: "base64",
      base64: "AAAgID",
      mime: "image/jpeg",
      width: 1024,
  ...
  it("defaults mime to image/png when base64 has no mime", () => {
    expect(parseImageGenResponse({ base64: "ZZZ" })).toBe(
