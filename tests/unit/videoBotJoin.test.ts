    ...
    setDocFields: async (
      ...
      for (const [k, v] of Object.entries(fields)) {
        if (k.startsWith("call.participants.")) {
          const call = ((doc as Record<string, unknown>).call ??= {
            participants: {},
          }) as {
...
describe("videoBotJoin", () => {
  ...
  it("admits the canonical Ugly Bot", async () => {
    const state = await videoBotJoin(
      fakeDb(humanCall),
      collections,
      "c1",
      "bot-ugly",
    );
    expect(state.participants["bot-ugly"]).toMatchObject({
      userId: "bot-ugly",
      isBot: true,
    });
  });
  ...
  it("rejects a non-bot id so a human cannot be joined as a fake participant", async () => {
    await expect(
      videoBotJoin(fakeDb(humanCall), collections, "c1", "w2ZBBoU5Jf83"),
  ...
  it("refuses to join when no call is active", async () => {
    await expect(
      videoBotJoin(fakeDb({}), collections, "c1", "bot-ugly"),
  ...
  it("refuses to join a call the last human already left", async () => {
    ...
    const ended = {
      call: { active: false, participants: {} },
    } as unknown as Partial<Conversation>;
    await expect(
      videoBotJoin(fakeDb(ended), collections, "c1", "bot-ugly"),
    ).rejects.toThrow("no active call");
  });
  ...
  it("refuses to join a roster holding only other bots", async () => {
    const botsOnly = {
      call: {
        active: true,
        participants: {
          "bot-sage": { userId: "bot-sage", isBot: true, joinedAt: 1 },
        },
      },
    } as unknown as Partial<Conversation>;
    await expect(
      videoBotJoin(fakeDb(botsOnly), collections, "c1", "bot-ugly"),
    ).rejects.toThrow("no active call");
  });
});
