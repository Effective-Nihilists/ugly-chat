...
export default function BotEditPage({
  ...
  const [linked] = useState(
    (): { characterId: string; thumbnail: string | null } | null => {
  ...
  const [characterThumbnail, setCharacterThumbnail] = useState<string | null>(
    linked?.thumbnail ?? null,
  );
  ...
  useEffect(() => {
    ...
    void socket
      ...
      .catch((err: unknown) => {
        console.error("[BotEdit] load failed", err);
      })
      .finally(() => {
        setLoaded(true);
      });
    ...
    async (thenChat: boolean) => {
      ...
      try {
        ...
        if (editId) {
          ...
          const res = (await socket.request("botCreate", payload)) as {
            botId: string;
          };
          id = res.botId;
        }
      ...
      >
        ...
        <div style={{ display: "flex", gap: 14, marginBottom: 18 }}>
          <ImageField
            label="Avatar"
            url={
              avatar.image.uri === defaultAvatar.image.uri
                ? null
                : avatar.image.uri
            }
            onChange={(url) => {
              setAvatar((a) =>
                url ? { ...a, uri: null, image: { uri: url } } : defaultAvatar,
              );
            }}
            round
            size={96}
        ...
        <Field
          label="Character"
          hint="A 3D character made on ugly.bot. Its thumbnail becomes this bot's picture in chat lists."
        >
          <div
            style={{
              display: "flex",
          ...
          >
            ...
            {characterThumbnail ? (
              <img
                {...crossOriginProps(characterThumbnail)}
            ...
            >
              {characterId
                ? `${name.trim() || "Bot"} — character linked`
                : "No character linked"}
            </span>
            <button
              type="button"
              onClick={() => {
                openUglyBotCharacterCreator(window.location.href, characterId);
              }}
              style={ghostBtnSmall}
        ...
        <Field
          ...
          hint="How the bot should behave, its persona, rules."
        >
          <AutoTextarea
            value={instruction}
        ...
        <Field
          label="First message"
          hint="The bot's opening greeting when a chat starts."
        >
          <AutoTextarea
            value={firstMessage}
        ...
        <Field
          label="Starter buttons"
          hint="Tappable buttons that send a preset prompt."
        >
            ...
            {buttons.map((b, i) => (
              ...
              >
                ...
                <input
                  ...
                  data-id="prompt-it-sends"
                />
                <button
                  type="button"
                  onClick={() => {
                    setButtons((p) => p.filter((_, j) => j !== i));
                  }}
                  aria-label="Remove"
                  style={iconBtn}
                  data-id="remove"
        ...
        >
          <button
            ...
            style={{ ...primaryBtn, opacity: !name.trim() || saving ? 0.5 : 1 }}
            data-id="button"
          >
          ...
          <button
            ...
            style={{ ...ghostBtn, opacity: !name.trim() || saving ? 0.5 : 1 }}
            data-id="button-2"
          >
            {editing ? "Save" : "Create"}
          </button>
    ...
    async (file: File | undefined) => {
      ...
      try {
        const processed = file.type.startsWith("image/")
          ? await downscaleImage(file, 1200)
          : file;
      ...
      <div
        ...
        style={{
          ...
          background: props.url
            ? `center / cover no-repeat url(${JSON.stringify(props.url)})`
            : "var(--app-tertiary)",
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
...
const input: React.CSSProperties = {
  ...
  background: "var(--app-input, var(--app-main))",
  color: "var(--app-foreground)",
  fontSize: 15,
  outline: "none",
