...
export default function ChatSettingsPage({
  ...
  const refetchMembers = useCallback(() => {
    ...
    void socket
      ...
      .then((res) => {
        setMembers((res as { members?: Member[] }).members ?? []);
      })
      .catch((err: unknown) => {
        console.error("[settings] members failed", err);
      });
  }, [socket, id]);
  ...
  const addByEmail = useCallback(async () => {
    ...
    try {
      ...
      if (r.status === "found") {
        await socket.request("conversationMemberAdd", {
          conversationId: id,
          userId: r.userId,
          role: "member",
        });
        ...
        setStatus(
          `No ugly.chat account for ${e} — they'll be invited when added via a new group.`,
        );
      }
      setAddEmail("");
    } catch (err) {
  ...
  const clearHistory = useCallback(() => {
    ...
    void socket
      ...
      .then(() => {
        router.push(":conversationId", { conversationId: id });
      })
      .catch((err: unknown) => {
  ...
  const leaveGroup = useCallback(() => {
    ...
    void socket
      ...
      .catch((err: unknown) => {
        console.error("[settings] leave failed", err);
        setStatus("Could not leave the group.");
      });
  ...
  const deleteConversation = useCallback(() => {
    ...
    void deleteOrLeaveConversation(socket, id, userId)
      ...
      .catch((err: unknown) => {
        ...
        setStatus("Could not delete the conversation.");
      });
  }, [id, socket, userId, router]);
      ...
      <div style={S.modal}>
        <div style={S.modalHead}>
          <span style={S.modalTitle}>
            {isBotChat ? "Bot info" : isDirect ? "Chat info" : "Group info"}
          </span>
          <button
            type="button"
            aria-label="Close"
        ...
        <div style={S.modalBody}>
          ...
          >
            <div style={idAvatar}>
              {isBotChat ? (
                <BotIcon size={26} style={{ opacity: 0.6 }} />
              ) : isDirect ? (
                <User size={26} style={{ opacity: 0.5 }} />
              ) : (
            ...
            {!isDirect ? (
              <div style={{ ...tokensRow, marginBottom: 4 }}>
                ...
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") void addByEmail();
                  }}
                  placeholder="invite someone — name@email.com"
                  spellCheck={false}
                  style={S.inputEl}
                  data-id="invite-someone-name-email"
                />
                <button
                  type="button"
          ...
          <div style={S.field}>
            ...
            <div style={srow}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={srowDesc}>
                  How big messages appear in every conversation.
                </div>
              </div>
            </div>
          ...
          <div style={S.field}>
            ...
            <div>
              ...
              <LinkRow
                ...
                desc="Plain markdown. Your words, your file."
              />
            </div>
          </div>
          ...
          <div style={S.field}>
            ...
            <div>
              ...
              <DangerRow
                ...
                desc={
                  isDirect
                    ? "Wipes this conversation for you."
                    : "Wipes it for you. Everyone else keeps theirs."
                }
                onClick={clearHistory}
                data-id="clear-history"
              />
              {!isOwner ? (
                <DangerRow
                  icon={<LogOut size={18} />}
                ...
                <DangerRow
                  icon={<Trash2 size={18} />}
                  title={isDirect ? "Delete conversation" : "Delete group"}
                  desc="Removes it for everyone. Can't be undone."
                  onClick={() => {
...
function LinkRow(props: {
  ...
  return (
    <div style={srow}>
      ...
      <ChevronRight
        size={16}
        style={{ color: "var(--app-foreground-muted)", flexShrink: 0 }}
      />
    </div>
  );
}
...
function DangerRow(props: {
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ ...srowTitle, color: "var(--app-error)" }}>
          {props.title}
        </div>
        {props.desc ? <div style={srowDesc}>{props.desc}</div> : null}
      </div>
...
const srowTitle: React.CSSProperties = {
  fontSize: 14,
  fontWeight: 600,
  color: "var(--app-foreground)",
};
const srowDesc: React.CSSProperties = {
  fontSize: 12,
  color: "var(--app-foreground)",
  opacity: 0.5,
  marginTop: 1,
