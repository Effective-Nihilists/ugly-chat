...
/** Row collapse/fade duration. Must match `.uc-row-exit` in styles.css. */
...
export function Sidebar(): React.ReactElement {
  ...
  // Conversations mid-delete: still rendered, collapsing out.
  const [exiting, setExiting] = useState<string[]>([]);
  const [expanded, setExpandedState] = useState(() =>
    loadBool("leftSidebarExpanded", true),
  );
  const [width, setWidth] = useState(() =>
    Math.max(MIN_WIDTH, Math.min(MAX_WIDTH, loadNum("leftSidebarWidth", 300))),
  );
  ...
  //
  // Play the exit BEFORE calling the server. The row is still in `filtered`
  // until the delete lands, so marking it `exiting` keeps it mounted long enough
  // to collapse and fade; deleting first would unmount it on the next tracked
  // update and the row would just blink out of existence (the reported issue).
  // The ~200ms delay before the request is imperceptible.
  const removeConversation = useCallback(
    (conversationId: string) => {
      ...
      window.setTimeout(() => {
        void deleteOrLeaveConversation(socket, conversationId, userId)
          .catch((err: unknown) => {
            console.error("[sidebar] delete failed", err);
          })
          .finally(() => {
            // Clear the marker either way — on failure the row must animate back
            // in rather than stay stuck mid-collapse.
            setExiting((prev) => prev.filter((id) => id !== conversationId));
          });
      }, ROW_EXIT_MS);
    },
    [socket, userId],
    ...
    return (
      <aside style={{ ...railStyle, width: COLLAPSED_WIDTH }}>
        ...
        <button
          ...
          onClick={openNew}
          className="uc-iconbtn"
          style={{ ...iconBtnStyle, alignSelf: "center", marginBottom: 6 }}
          data-id="button-2"
        >
          <Plus size={20} />
        </button>
