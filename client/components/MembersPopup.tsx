...
export function MembersPopup({
  ...
  const load = useCallback(async () => {
    try {
      const res = (await socket.request("conversationMembers", {
        conversationId,
      })) as {
        members?: Member[];
      };
  ...
  const openAdd = useCallback(async () => {
    ...
    try {
      const res = (await socket.request("userContacts", {})) as {
        users?: Contact[];
      };
                        ...
                        <button
                          ...
                          onClick={() =>
                            void act(
                              ...
                              { conversationId, userId: m.userId },
                              `rm-${m.userId}`,
                            )
                          }
                          style={iconBtn(false)}
            ...
            confirmDelete ? (
              <div
                style={{
                  ...
                  padding: "8px 10px",
                  borderRadius: 10,
                  background: "rgba(var(--app-error-rgb, 220,38,38), 0.08)",
                  border: "1px solid var(--app-error)",
                }}
              >
                <span
                  style={{ flex: 1, fontSize: 13, color: "var(--app-error)" }}
                >
          ...
          <Hint
            text={
              props.contacts.length === 0
                ? "No contacts yet — chat with people first."
