...
export function NewChatPopup({
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      setStatus(
        msg && msg.length > 0 && !msg.startsWith("[")
          ? msg
  ...
  const soleEmail =
    count === 1 && selected.length === 0 ? allEmails()[0] : undefined;
      ...
      <div style={modalBody}>
        ...
        <div style={field}>
          ...
          <div style={tokens}>
            <Mail
              size={16}
              style={{ opacity: 0.5, flexShrink: 0, alignSelf: "center" }}
            />
            {selected.map((uid) => (
              <span
                key={uid}
            ...
            {emails.map((e) => {
              ...
              return (
                ...
                >
                  {found ? (l.name ?? e) : e}
                  {l?.status === "invite" ? (
                    <span style={inviteTag}>invite</span>
                  ) : null}
                  <button
                    type="button"
                    aria-label={`Remove ${e}`}
                    ...
                    style={found ? { ...chipX, color: "#fff" } : chipX}
                    data-id="button"
                  >
                    <X size={11} />
                  </button>
            ...
            <input
              ...
              onKeyDown={onKeyDown}
              onBlur={() => {
                if (draft.trim() && draftValid) addChip(draft);
              }}
              placeholder={
                emails.length === 0 && selected.length === 0
                  ? "tap a person below, or type an email…"
                  : "add another…"
              }
              spellCheck={false}
              autoFocus
          ...
          {draftValid && draftLookup ? (
            <div style={hint} data-id="email-lookup">
              {draftLookup.status === "checking" ? (
                ...
                <>
                  <b>{draftLookup.name}</b> is on ugly.chat — this opens a chat
                  with them.
                </>
              ) : draftLookup.status === "invite" ? (
                <>
                  Not on ugly.chat — we&rsquo;ll <b>email them an invite</b>. No
                  chat opens until they join.
                </>
              ) : (
          ...
          <div style={hint}>
            Tap someone below, or type an email. One person starts a 1:1; two or
            more makes a group.
          </div>
        </div>
          ...
          <div style={field}>
            ...
            <div style={inputRow}>
              ...
              <input
                ...
                spellCheck={false}
                style={inputEl}
                data-id="ship-crew-prod-incidents"
              />
            </div>
            <div style={hint}>
              Everyone you add sees the history <b>from the moment they join</b>{" "}
              — nothing before.
            </div>
          </div>
        ) : null}
        ...
        <div style={field}>
          ...
          <label style={fieldLabel}>
            Recent — people you&rsquo;ve chatted with
          </label>
          <div style={contactsBox}>
            {contactsLoading ? (
              <div style={contactsMsg}>Loading…</div>
            ) : contacts.length === 0 ? (
              <div style={contactsMsg}>
                No contacts yet — invite someone by email above.
              </div>
            ) : (
              contacts.map((c) => {
        ...
        {status ? (
          <div
            style={{
              fontSize: 13,
              fontWeight: 600,
              color: statusError ? "var(--app-error)" : "var(--app-primary)",
            }}
          >
            {status}
          </div>
